...
protocol FollowsServiceType {
    ...
    func count(on request: Request, sourceId: UUID) async throws -> Int
    
    /// Returns list of following accoutns.
    func following(on request: Request, sourceId: UUID, page: Int, size: Int) async throws -> Page<User>

    /// Returns amount of followers.
    func count(on request: Request, targetId: UUID) async throws -> Int
    
    /// Returns list of account that follow account.
    func follows(on request: Request, targetId: UUID, page: Int, size: Int) async throws -> Page<User>
}

final class FollowsService: FollowsServiceType {

    public func count(on request: Request, sourceId: UUID) async throws -> Int {
    ...
    public func following(on request: Request, sourceId: UUID, page: Int, size: Int) async throws -> Page<User> {
    ...
    public func count(on request: Request, targetId: UUID) async throws -> Int {
        return try await Follow.query(on: request.db).group(.and) { queryGroup in
    ...
    public func follows(on request: Request, targetId: UUID, page: Int, size: Int) async throws -> Page<User> {
        return try await User.query(on: request.db)
