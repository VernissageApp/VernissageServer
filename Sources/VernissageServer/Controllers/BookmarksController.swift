...
extension BookmarksController: RouteCollection {
    
    @_documentation(visibility: private)
    static let uri: PathComponent = .constant("bookmarks")
    
    func boot(routes: RoutesBuilder) throws {
        let timelinesGroup = routes
            .grouped("api")
            .grouped("v1")
            .grouped(BookmarksController.uri)
...
/// Returns user's private bookmarks.
///
/// This is the controller that is responsible for managing the list of bookmarks
...
struct BookmarksController {
    ...
    ///
    /// This is an endpoint that returns a list of statuses which has been bookmarked by the user.
    ///
    ...
    ///
    /// > Important: Endpoint URL: `/api/v1/bookmarks`.
    ///
    /// **CURL request:**
    ///
    /// ```bash
    /// curl "https://example.com/api/v1/bookmarks" \
    /// -X GET \
