...
extension Application {
    ...
    func clearFiles(attachments: [Attachment]) {
        for attachment in attachments {
            let orginalFileUrl = URL(fileURLWithPath: "\(self.directory.workingDirectory)/Public/storage/\(attachment.originalFile.fileName)")
            try? FileManager.default.removeItem(at: orginalFileUrl)
            
            let smalFileUrl = URL(fileURLWithPath: "\(self.directory.workingDirectory)/Public/storage/\(attachment.smallFile.fileName)")
