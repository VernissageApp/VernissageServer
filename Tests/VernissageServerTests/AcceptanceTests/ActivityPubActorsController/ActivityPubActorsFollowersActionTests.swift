...
extension ControllersTests {
    ...
    struct ActivityPubActorsFollowersActionTests {
        ...
        @Test
        func `Followers information should be returned for existing actor`() async throws {
            ...
            #expect(orderedItemIds.count == 2, "Followers list should contain two items.")
            #expect(orderedItemIds.contains("http://localhost:8080/actors/karolbrzuch"), "Follower 'karolbrzuch' should be visible on list.")
            #expect(orderedItemIds.contains("http://localhost:8080/actors/weronikabrzuch"), "Follower 'weronikabrzuch' should be visible on list.")
        }
        
        @Test
        func `First property should not be set for actors without followers`() async throws {
        ...
        @Test
        func `Followers information should not be returned for not existing actor`() async throws {
            
            // Act.
            let response = try await application.sendRequest(to: "/actors/unknown/followers", method: .GET)
        ...
        @Test
        func `Followers data should be returned for existing actor`() async throws {
            // Arrange.
            ...
            #expect(orderedCollectionDto.totalItems == 2, "Property 'totalItems' is not valid.")
            let orderedItemIds = orderedCollectionDto.orderedItems.objects().map({ $0.id })
            #expect(orderedItemIds.contains("http://localhost:8080/actors/karolcent"), "Followers 'karoltram' should be visible on list.")
            #expect(orderedItemIds.contains("http://localhost:8080/actors/weronikacent"), "Followers 'weronikatram' should be visible on list.")
        }
